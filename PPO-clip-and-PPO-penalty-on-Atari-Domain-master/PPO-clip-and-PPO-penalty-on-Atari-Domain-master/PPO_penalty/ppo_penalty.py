...
import gym
import time
import spinup.algos.ppo.core_2 as core_2
...
class PPOBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        self.obs_buf = np.zeros(core_2.combined_shape(size, obs_dim[0]*obs_dim[1]*obs_dim[2]), dtype=np.float32)
        self.act_buf = np.zeros(core_2.combined_shape(size, act_dim), dtype=np.float32)
        self.adv_buf = np.zeros(size, dtype=np.float32)
        self.rew_buf = np.zeros(size, dtype=np.float32)
        self.ret_buf = np.zeros(size, dtype=np.float32)
        self.val_buf = np.zeros(size, dtype=np.float32)
        self.logp_buf = np.zeros(size, dtype=np.float32)
        self.logp_all_buf = np.zeros(core_2.combined_shape(size, 18), dtype=np.float32)
        self.gamma, self.lam = gamma, lam
        self.ptr, self.path_start_idx, self.max_size = 0, 0, size

    def store(self, obs, act, rew, val, logp, logp_all_buf):
        """
        ...
        #print(logp_all_buf[0])
        self.logp_all_buf[self.ptr] = logp_all_buf[0]
        self.ptr += 1
    ...
    def finish_path(self, last_val=0):
        ...
        self.adv_buf[path_slice] = core_2.discount_cumsum(deltas, self.gamma * self.lam)

        # the next line computes rewards-to-go, to be targets for the value function
        self.ret_buf[path_slice] = core_2.discount_cumsum(rews, self.gamma)[:-1]
    ...
    def get(self):
        ...
        print(self.logp_all_buf.shape)
        print(self.logp_buf.shape)
        return [self.obs_buf, self.act_buf, self.adv_buf,
                self.ret_buf, self.logp_buf, self.logp_all_buf]


#  上面是一个类，定义了PPO buffer
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='Alien-v4')
    ...
    ppo(lambda: gym.make(args.env), actor_critic=core_2.mlp_actor_critic,
